...
function InputBorderLess({ label, value, prefix, onChange }) {
    ...
    return (
        <View style={styles.wrapper} onMouseOver={() => { setFocus(true) }} >
            ...
            {focus ? <input onBlur={() => setFocus(false)} style={{ border: 'none', backgroundColor: 'green', width: ((data.length * 7.4 / 10) > 10 ? (data.length * 7.4 / 10) : 10) + 'rem' }} value={data} onChange={onChangeData} /> : <Text style={globalStyles.text}>{value}</Text>}
